...
<div class="col-md-8">
 <div class="panel panel-primary">
            <div class="panel-heading">Area Group Lists</div>
            <div class="panel-body">
    <div class="col-md-12" style="padding-bottom:10px;">
                    {!! Html::link("area-group/create",'Create Area Group', ['class' => 'btn btn-success','target'=>'_blank']) !!}
...
<table class="table table-bordered table-striped table-hover category-table" data-toggle="dataTable" data-form="deleteForm">
    ...
    <tbody>
        @foreach($group_all as $row)
            <tr>
                <td>{{ $row->id }}</td>
                <td>{{ $row->main_pop }}</td>                
                <td>{{ $row->secondary_pop }}</td>                
                <td>{!! Html::link("area-group/$row->id/edit", ' ',['class' => 'iframe glyphicon glyphicon-edit']) !!}</td>
