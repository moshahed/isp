 ...
 <div class="panel panel-primary">
            ...
            <div class="panel-body">
                <div class="col-md-12" style="padding-bottom:10px;">
                    {!! Html::link("secondary-pop/create",'Create Secondary POP', ['class' => 'btn btn-success','target'=>'_blank']) !!}
...
<table class="table table-bordered table-striped table-hover category-table" data-toggle="dataTable" data-form="deleteForm">
    ...
    <tbody>
        @foreach($secondary_pop_list as $row)
            <tr>
                <td>{{ $row->id }}</td>
                <td>{{ $row->name }}</td>                
                <td>{{ $row->MainPop->name }}</td>                
                              
                <td>{!! Html::link("secondary-pop/$row->id/edit", ' ',['class' => 'iframe glyphicon glyphicon-edit']) !!}</td>
